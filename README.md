...
## ♾️ FiveM Deployment Checklist

- Run `pnpm build` after every code change to refresh the compiled `build/client.js` and `build/server.js` files referenced by the resource.
- Verify that `web/dist` exists (generated by the build step) so the NUI page defined in `fxmanifest.lua` can load.
- Place the entire `summit_phone` folder inside your FiveM `resources/` directory.
- Add `ensure summit_phone` to your `server.cfg` once the build artifacts are in place.
