...
interface Conversation {
    otherUser: TweetProfileData;
    lastMessage: PrivateMessage;
    unreadCount: number;
}

export default function Messages(props: { show: boolean; onClose: () => void; onStartChat: (user: TweetProfileData) => void }) {
    ...
    const loadConversations = async () => {
        setLoading(true);
        try {
            const res = await fetchNui('pigeon:getConversations', phoneSettings.pigeonIdAttached);
            if (res && typeof res === 'string') {
                setConversations(JSON.parse(res));
            }
        } catch (error) {
                    ...
                    conversations.map((conversation, index) => (
                        <div
                            ...
                            style={{
                                ...
                                marginBottom: '0.89vh',
                                cursor: 'pointer',
                                border: conversation.unreadCount > 0 ? '1px solid #0A84FF' : '1px solid transparent',
                            }}
